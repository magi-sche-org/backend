...
var UserWhere = struct {
	...
	IsRegistered: whereHelperbool{field: "`user`.`is_registered`"},
...
func (r *userR) GetOwnerEvents() EventSlice {
	if r == nil {
		return nil
	}
...
func (r *userR) GetUserEventAnswers() UserEventAnswerSlice {
	if r == nil {
		return nil
	}
...
var (
	userAllColumns            = []string{"id", "name", "is_registered", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"id", "name"}
	userColumnsWithDefault    = []string{"is_registered", "created_at", "updated_at"}
...
func (o *User) RefreshTokens(mods ...qm.QueryMod) refreshTokenQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`refresh_token`.`user_id`=?", o.ID),
	)
...
func (o *User) UserEventAnswers(mods ...qm.QueryMod) userEventAnswerQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`user_event_answer`.`user_id`=?", o.ID),
	)

	return UserEventAnswers(queryMods...)
}

// LoadOwnerEvents allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadOwnerEvents(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`event`),
		qm.WhereIn(`event.owner_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.OwnerEvents = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.OwnerID {
				local.R.OwnerEvents = append(local.R.OwnerEvents, foreign)
				if foreign.R == nil {
					foreign.R = &eventR{}
				}
...
func (userL) LoadRefreshTokens(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`refresh_token`),
		qm.WhereIn(`refresh_token.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load refresh_token")
	}

	var resultSlice []*RefreshToken
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice refresh_token")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on refresh_token")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for refresh_token")
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserEventAnswers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`user_event_answer.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// AddOwnerEvents adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.OwnerEvents.
...
func (o *User) AddOwnerEvents(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Event) error {
	...
	if o.R == nil {
		...
		o.R.OwnerEvents = append(o.R.OwnerEvents, related...)
	}
